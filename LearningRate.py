...
def DNN(images_train, labels_train, images_test, labels_test):
    ...
    for eta in np.arange(start=0.01, stop=0.511, step=0.05):
        for epochs in range(finish):
            ...
            print("{0} / {1} epochs (learning rate = {2})".format(epochs+1, finish, eta))
...
def show(Y):
    X = np.arange(start=0.01, stop=0.511, step=0.05)
    plt.figure()
    plt.plot(X, Y)
